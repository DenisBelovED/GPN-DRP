...
from core.configuration import NMS_IOU_THRESHOLD, CONFIDENCE_THRESHOLD, MAX_NMS_BOXES
...
class NMS(object):
    ...
    def nms(self, boxes, box_scores):
        ...
        for i in range(1, self.num_class):
            ...
            if not tf.size(box_of_class):
                continue
            score_of_class = box_scores[:, i][mask[:, i]]
        ...
        box_tensor = tf.concat(box_list, axis=0)
        score_tensor = tf.concat(score_list, axis=0)
        class_tensor = tf.concat(class_list, axis=0)
